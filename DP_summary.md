...
给定一个字符串，求字符串中回文子串的数目，注意这里的回文子串，是指连续的子串，而不是子序列

- dp数组定义：`dp[i][j]`表示字符串`s[i:j]`是否是回文串
- 递推公式确定，根据判断的`s[i]`和`s[j]`是否相等,分为两种情况
    - 如果`s[i] != s[j]`，很简单,不是回文子串,`dp[i][j] = false`
    - 如果`s[i] == s[j]`，需要再次分为两种情况考虑
        - 如果`j - i <= 1`，说明只有一个字符或者两个字符，直接是回文子串
        - 如果`j - i >= 2`，需要判断`dp[i+1][j-1]`是否是回文子串(ture),如果是,`dp[i][j] = true`
- dp数组初始化：这里和最长回文子序列有所不同,因为当前会判断一次j-i是否小于等于1,所以单个元素和两个元素的情况,会在遍历中正确更新,不需要初始化
  - 对应的遍历情况和回文子序列有区别:
    - 回文子串：j从`j=i`前往后遍历
    - 回文子序列：j从`j=i+1`后往前遍历
- dp数组遍历方向：根据递推公式可看到需要对`dp[i+1][j-1]`进行判断,`dp[i+1][j-1]`在`dp[i][j]`的左下角,所以遍历的时候需要i从后往前,j需要从前往后
- 最后统计回文子串的个数,只需要统计`dp[i][j]`为true的个数即可
...
int countSubstrings(string s) {
      ...
      for (int i = s.size() - 1; i >= 0; --i) {
        //从s的右边开始向左边遍历
        for (int j = i; j < s.size(); ++j) {//注意j初始化为i
          //每一各部分，从当前部分向后遍历，判断字串是否是回文
          if (s[i] == s[j] && (j - i <= 1 || dp[i + 1][j - 1])) {
            //需要首先判断，j - i <= 1,防止dp[i + 1][j - 1]越界
            res++;
            dp[i][j] = true;
          }
        }
...
给定一个字符串`s`，找到其中最长的回文子序列的长度。并返回这个长度.注意这里的回文子序列，是指不连续的子序列，而不是连续的子串

- dp数组定义：`dp[i][j]`表示字符串`s[i:j]`的最长回文子序列的长度
- 递推公式确定，根据判断的`s[i]`和`s[j]`是否相等,分为两种情况,**注意这里和回文子串的区别,就是不连续的子序列**
    - 如果`s[i] == s[j]`，找到了相同的字符,那直接原结果加2,`dp[i][j] = dp[i+1][j-1] + 2`
    - 如果`s[i] != s[j]`，说明`s[i]`和`s[j]`不相等,那么就分别加入`s[i]`和`s[j]`，看哪个更长,`dp[i][j] = max(dp[i+1][j], dp[i][j-1])`
- dp数组初始化：这里和回文子串有所不同,从条件可以看出,没有j-i>=1这个条件了,所以如果i==j,没法正确初始化
  - 对应的遍历情况和回文子串有区别:
    - 回文子串：j从`j=i`前往后遍历
    - 回文子序列：j从`j=i+1`后往前遍历
- dp数组遍历方向：根据递推公式可看到需要对`dp[i+1][j-1]`进行判断,`dp[i+1][j-1]`在`dp[i][j]`的左下角,所以遍历的时候需要i从后往前,j需要从前往后
...
int longestPalindromeSubseq(string s) {
      ...
      for (int i = s.size() - 1; i >= 0; --i) {
        for (int j = i + 1; j < s.size(); ++j) {//注意这里j初始化为i+1
          if (s[i] == s[j])
            dp[i][j] = dp[i + 1][j - 1] + 2;
          else
